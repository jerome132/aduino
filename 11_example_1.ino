...
#define _DUTY_MIN 553.0 // servo full clockwise position (0 degree)
#define _DUTY_NEU 1476.0 // servo neutral position (90 degree)
#define _DUTY_MAX 2399.0 // servo full counterclockwise position (180 degree)

float duty = _DUTY_MAX/(_DUTY_MAX - _DUTY_MIN);
...
void loop() {
  ...
  float dummy = (dist_raw - _TARGET_LOW)/_TARGET_LOW;
  dummy = _DUTY_MIN + dummy * (_DUTY_MAX - _DUTY_MIN);
  ...
  if (dist_raw <= _TARGET_LOW) {
    ...
    myservo.writeMicroseconds(dummy);
  } else {
